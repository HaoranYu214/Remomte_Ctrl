...
Run python -m pip install -e . from the repository root for an editable installation. Measurement entries add src to their import path and can usually run directly. Experiment parameters live in [measurements](../measurements/README.md), and multi-step tasks in [workflows](../measurements/workflows/README.md).
...
在仓库根目录运行 python -m pip install -e . 可做可编辑安装。测量入口会加入 src 路径，通常也可直接运行。实际实验参数在 [measurements](../measurements/README.md)，多步任务在 [workflows](../measurements/workflows/README.md)。
