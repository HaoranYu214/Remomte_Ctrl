...
# Start here: PV/PUND + map + IV workflow; edit stage parameters, RUN_ORDER and BASE_SAVE_DIR.
# Stage settings are grouped in PV2_PARAMS, PUND_PARAMS, PV2_PUND_MAP, IV_TESTS and SEGMENTED_IV.
# Flow: main -> preview or acquire via PREVIEW_ONLY -> existing stage entries -> checkpoint each stage.
# configure_map/configure_iv update child-module settings in memory; this workflow owns run settings.
...
def load_test_modules():
    ...
    return {
        ...
        "iv": importlib.import_module("measurements.smu.2terminal.segmented_voltage_sweep"),
    }


# Clear and update a dictionary in place so existing references receive the new configuration.
...
# Apply this workflow's sweep lists, parameters and output settings to the child map module in memory.
...
# Convert peak voltage into a symmetric IV path: 0 -> +Vmax -> 0 -> -Vmax -> 0.
...
# Apply this workflow's instrument, output path, turning points and parameters to the child IV module.
...
# Check stage order, repeats, map lists and IV point counts; return test sizes without hardware access.
def validate_package_config(modules):
    ...
    for iv_test, name in zip(IV_TESTS, iv_names):
        ...
        sweep_values = modules["iv"].build_points(
...
# Repeat PV2/PUND for this stage and retain accepted current ranges for later runs.
...
# Configure and execute the PV2/PUND map, collect accepted ranges and propagate measurement failures.
...
# Configure and repeat each IV test, waiting the configured interval between acquisitions.
...
# Execute PV/PUND, map and IV stages in RUN_ORDER; checkpoint status and return a DataFrame.
# This function acquires data; main selects it or preview_package using PREVIEW_ONLY.
def run_package():
...
# Use PREVIEW_ONLY to select offline preview or acquisition for the full package.
def main():
    if PREVIEW_ONLY:
