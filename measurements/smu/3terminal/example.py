...
"""Generic three-terminal SMU example: define channels -> outputs -> sampling -> execute -> read."""
# Terminals are named CH1/CH2/CH3; their roles depend on the physical wiring.
# By default, CH1 sweeps voltage, CH2 holds 0.1 V, and CH3 holds 0 V; read all three voltages and currents.
...
KXCI_PLOT = False  # True is for hardware trials: select six LI variables, then DM1; verify that all six remain valid.
...
def configure(query, values):
    ...
    active = normalize_channels((CHANNEL_1, CHANNEL_2, CHANNEL_3))
    ...
    if any(not math.isfinite(float(v)) or abs(float(v)) > 210
           for v in [*values, PARAMS["ch2_voltage"], PARAMS["ch3_voltage"]]):
    ...
    query(f"CH{CHANNEL_3}, 'V{CHANNEL_3}', 'I{CHANNEL_3}', 1, 3")
    query("SS")
    ...
    query(f"VC{CHANNEL_3}, {PARAMS['ch3_voltage']}, {PARAMS['ch3_current_compliance']}")
    ...
    if KXCI_PLOT:
        ...
        # Six-variable readback still needs hardware verification; retain all variables and point-count checks.
        query("SM DM1")
        query(f"XN 'V{CHANNEL_1}', 1, {min(values):g}, {max(values):g}")
        limit1 = PARAMS["ch1_current_compliance"]
        limit2 = PARAMS["ch3_current_compliance"]
        query(f"YA 'I{CHANNEL_1}', 1, {-limit1:g}, {limit1:g}")
        query(f"YB 'I{CHANNEL_3}', 1, {-limit2:g}, {limit2:g}")
    return variables
...
def main():
    ...
    if PREVIEW_ONLY:
        ...
        return preview_channel_voltages({
            ...
            f"CH{CHANNEL_3}": [PARAMS["ch3_voltage"]] * expected_points,
        }, title="3terminal channel voltage preview")
    ...
    with SMUSession(INST) as session:
        ...
        try:
            ...
            rpm_targets = connect_smus_to_probes(query, (CHANNEL_1, CHANNEL_2, CHANNEL_3), SMU_CONNECTIONS)
            variables = configure(query, values)
            raise_for_kxci_error(query, context="Three-terminal setup")
            execute_and_wait(query, timeout_s=timeout)
            data = retrieve_variables(query, variables, expected_point_count=expected_points)
            raise_for_kxci_error(query, context="Three-terminal readout")
            restore_rpms_to_pulse(query, rpm_targets)
        finally:
            shutdown_system_mode(query, AVAILABLE_CHANNELS)

    stem = reserve_output_stem(SAVE_DIR, "3terminal")
